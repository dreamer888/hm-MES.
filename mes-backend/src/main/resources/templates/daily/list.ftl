        ...
        <form id="js-search-form" class="layui-form" lay-filter="js-q-form-filter">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <label class="layui-form-label"  style="width: fit-content"    > 订单编码(包含)</label>
                    <div class="layui-input-inline">
                        <input type="orderCode" name="orderCode" autocomplete="off" class="layui-input"   >
                    </div>
                </div>
                <div class="layui-inline"    >
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/daily/plan/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    ...
                    field: 'planQty', title: '当日计划产量'
                },  {
                    field: 'makedQty', title: '当日实际产量'
                }, {
                    field: 'badQty', title: '当日次品数量'
                },  {
                    field: 'passRate', title: '一次通过率'
                },{
                    field: 'realPieceTime', title: '每件实际耗时'
                }, {
                    fixed: 'right',
                    field: 'operate',
                    title: '操作',
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/daily/plan/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/daily/plan/delete',
                        async: false,
                        type: 'POST',
                        // 是否显示 loading
                        showLoading: true,
