        ...
        <form id="js-search-form" class="layui-form" lay-filter="js-q-form-filter">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <label class="layui-form-label"  style="width: fit-content">  生产线  </label>
                    <div class="layui-input-inline">
                        <input type="lineId" name="lineIdLike" autocomplete="off" class="layui-input"  style="width:300px "  >
                    </div>
                </div>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/config/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    field: 'lineId', title: '生产线'
                }, {
                    field: 'serverIp', title: 'server ip'
                }, {
                    field: 'serverPort', title: 'server port'
                }, {
                    field: 'interv', title: '刷新间隔'
                }, {
                    field: 'type', title: '通讯类型'
                },  {
                    field: 'useComm', title: '是否使用串口'
                },
                    {
                    fixed: 'right',
                    field: 'operate',
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/config/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/config/delete',
                        async: false,
                        type: 'POST',
                        // 是否显示 loading
                        showLoading: true,
