        ...
        <form id="js-search-form" class="layui-form" lay-filter="js-q-form-filter">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <label class="layui-form-label"  style="width: fit-content"> 设备名称(包含)</label>
                    <div class="layui-input-inline">
                        <input type="device" name="deviceLike" autocomplete="off" class="layui-input"  style="width:300px "  >
                    </div>
                </div>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/device/device/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/device/device/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/device/device/delete',
                        async: false,
                        type: 'POST',
                        // 是否显示 loading
                        showLoading: true,
