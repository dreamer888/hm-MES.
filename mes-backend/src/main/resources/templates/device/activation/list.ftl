        ...
        <form id="js-search-form" class="layui-form" lay-filter="js-q-form-filter">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <label class="layui-form-label"  style="width: fit-content"> 设备(包含)</label>
                    <div class="layui-input-inline">
                        <input type="text" name="deviceLike" autocomplete="off" class="layui-input"   >
                    </div>
                </div>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/device/activation/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    ...
                    field: 'planTime', title: '计划时长'
                }, {
                    field: 'workTime', title: '工作时长'
                }, {
                    field: 'badTime', title: '故障时长'
                },
                    {
                        field: 'waitTime', title: '等待时长'
                    },
                    {
                        field: 'activationRate', title: '时间稼动率'
                    },
                    , {
                    field: 'flag', title: '设备状态 '
                }
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/device/activation/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/device/activation/delete',
                        async: false,
                        type: 'POST',
                        // 是否显示 loading
                        showLoading: true,
