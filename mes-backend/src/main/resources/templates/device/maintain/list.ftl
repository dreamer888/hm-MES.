...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/device/maintain/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    ...
                    field: 'errorDesc', title: '故障描述'
                }, {
                    field: 'dealer', title: '处理人'
                }, {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'add') {
                var index = spLayer.open({
                    title: '添加',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/device/maintain/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/device/maintain/delete',
                        async: false,
                        type: 'POST',
                        // 是否显示 loading
                        showLoading: true,
